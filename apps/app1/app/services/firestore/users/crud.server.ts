...
export const UserDb = ({
  ...
  const readListOfUsers = async ({ userList }: { userList: string[] }) => {
    const p = userList.map((userId) => read({ id: userId }));

    const userDocsSettled = await Promise.allSettled(p);

    return userDocsSettled;
  };
